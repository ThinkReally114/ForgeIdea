...
主人希望「基于 opencode 的理念」在安卓端做一个移动端编程辅助 agent。本项目不直接移植 opencode 代码，而是借鉴其**可扩展架构**（rule + 权限 + MCP + skills + LSP + 工具），用原生 Kotlin + Jetpack Compose 实现一个独立 app，UI 风格参考 QZMusic v2（毛玻璃 + 动态取色 + 渐变背景 + 胶囊组件 + Lottie 动效），模型接入 OpenCode Zen 免费模型（OpenAI 兼容 API）。
...
| 风格要素 | 实现方式 | 依赖 |
|---|---|---|
| 网格渐变背景 | 多色流动渐变作底色 | ComposeMeshGradient |
...
**预设主题切换：** 提供 3-4 个预设主题（如「QZ 紫」「深空蓝」「极光绿」「暖橙」），每个主题预设好渐变色、品牌色、强调色组合。用户在设置页一键切换，不提供细粒度滑块。
...
| 视觉增强 | Haze / ComposeMeshGradient / M3Color / Capsule / dotlottie | 复刻 QZMusic DNA |
